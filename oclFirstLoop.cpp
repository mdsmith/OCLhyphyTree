...
int main(int argc, char **argv)
{
	...
	ciErr1 = clGetDeviceInfo(cdDevice, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t), &maxWorkGroupSize, NULL);
	if (ciErr1 != CL_SUCCESS)
	{
	...
	cl_uint extcheck;
	ciErr1 = clGetDeviceInfo(cdDevice, CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE, sizeof(cl_uint), &extcheck, NULL);
	if (extcheck ==0 ) {
	...
	cmNode_cache = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY, sizeof(clfp) * CHARACTERS * SITES, NULL, &ciErr1);
	cmModel = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY, sizeof(clfp) * CHARACTERS * CHARACTERS, NULL, &ciErr2);
    ciErr1 |= ciErr2;
	cmParent_cache = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, sizeof(clfp) * CHARACTERS * SITES, NULL, &ciErr2);
    ciErr1 |= ciErr2;
...
//										   &szKernelLength, &ciErr1);
    ...
    ciErr1 = clEnqueueWriteBuffer(cqCommandQueue, cmNode_cache, CL_FALSE, 0, sizeof(clfp) * CHARACTERS * SITES, node_cache, 0, NULL, NULL);
    ciErr1 |= clEnqueueWriteBuffer(cqCommandQueue, cmModel, CL_FALSE, 0, sizeof(clfp) * CHARACTERS * CHARACTERS, model, 0, NULL, NULL);
	ciErr1 |= clEnqueueWriteBuffer(cqCommandQueue, cmParent_cache, CL_FALSE, 0, sizeof(clfp) * CHARACTERS * SITES, parent_cache, 0, NULL, NULL);
